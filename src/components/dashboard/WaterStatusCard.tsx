...
const WaterStatusCard: React.FC<WaterStatusCardProps> = ({ status, metrics, isChecking, descriptionOverride, history }) => {
  ...
  return (
    <div 
        className={`relative bg-slate-900/50 backdrop-blur-xl rounded-2xl shadow-lg ${config.borderColor} overflow-hidden transition-all duration-500 ${isUnsafe ? 'animate-crackling-glow' : `shadow-[0_0_25px_rgba(var(--${config.colorClass}-rgb),0.2)]`} hover:-translate-y-1 hover:shadow-[0_0_40px_rgba(var(--${config.colorClass}-rgb),0.5)]`}
        style={{'--glow-color': `var(--${config.colorClass}-rgb)`} as React.CSSProperties}
    >
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-grow text-center sm:text-left">
            <h2 className={`text-2xl font-bold ${config.titleColor} ${
                status === 'safe' ? 'text-glow-cyan' :
                status === 'caution' ? 'text-glow-yellow' : 'text-glow-red'
            } ${isUnsafe ? 'animate-subtle-flicker' : ''}`}>{config.title}</h2>
        ...
        {history && history.length > 0 && (
             <div className="mt-6 pt-6 border-t border-slate-700/60">
                <div className="flex items-center justify-center gap-2 mb-4">
                    ...
                    <h4 className="font-semibold text-cyan-300">{t('healthWorker.history')}</h4>
                </div>
