...
import { DiseaseTrend, Village } from './constants';
 ...
 */
export async function analyzeWaterImage(imageDataBase64: string, mimeType: string): Promise<{ status: 'safe' | 'caution' | 'unsafe'; explanation: string }> {
  try {
    const imagePart = {
      inlineData: {
        mimeType,
 ...
 * @returns A string containing the forecast summary.
 * @throws {ForecastError} If the API call fails or returns an empty response.
 */
export async function generateOutbreakForecast(trends: DiseaseTrend[]): Promise<string> {
    const prompt = `
        You are an AI epidemiologist for a public health app.
        Based on this data for Northeast India, generate a 2-sentence outbreak forecast summary.
        Data: ${JSON.stringify(trends)}
        Focus on the highest risk. Be concise and direct.
    `.trim();
