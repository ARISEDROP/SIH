...
import { GoogleIcon, FacebookIcon, MailIcon, LockIcon, UserIcon, UserGroupIcon } from './icons';
        ...
        <div className="px-8 pb-8">
            ...
            <div className="relative my-6">
                ...
                <div className="relative flex justify-center">
                    <span className="bg-slate-900/20 px-2 text-sm text-gray-400 backdrop-blur-sm">Or continue with</span>
                </div>
            </div>
            
            <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); onLogin(selectedRole); }}>
                <InputField icon={<MailIcon />} type="email" placeholder="Email" required />
                <InputField icon={<LockIcon />} type="password" placeholder="Password" required />
                
                <div className="space-y-3 pt-2">
                    <p className="text-sm font-medium text-cyan-200 text-center">Select your role:</p>
             ...
             <p className="mt-6 text-center text-sm text-gray-400">
                No account? <a href="#" className="font-semibold text-cyan-400 hover:text-cyan-300 transition-colors duration-300 hover:underline underline-offset-4">Sign up</a>
            </p>
        </div>
      </div>
    </div>
...
const RoleToggle: React.FC<{icon: React.ReactElement<any>, label: string, isActive: boolean, onClick: () => void}> = ({icon, label, isActive, onClick}) => (
    <button 
        ...
        className={`flex items-center justify-center gap-2 p-3 rounded-lg border-2 text-sm font-semibold transition-all duration-300 active:scale-[0.98] ${
            isActive 
            ? 'bg-cyan-500/30 border-cyan-500 text-white shadow-md shadow-cyan-500/10' 
