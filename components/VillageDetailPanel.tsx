...
import { Village, WaterStatus, QualityHistory } from '../constants';
import { XIcon, ClockIcon, ShieldCheckIcon, AlertTriangleIcon, AlertOctagonIcon } from './icons';
...
// Generate mock history for demonstration purposes
...
const VillageDetailPanel: React.FC<VillageDetailPanelProps> = ({ village, onClose, onRecenter }) => {
    const config = statusConfig[village.status];
    const history = generateMockHistory(village.status);

    return (
        <div className="bg-slate-900/95 backdrop-blur-lg p-4 rounded-l-2xl border-l border-t border-b border-slate-700/80 h-full flex flex-col shadow-2xl">
            ...
            <div className={`p-3 rounded-lg flex items-center gap-3 ${config.bgClass} mb-4`}>
                {React.cloneElement(config.icon, { className: 'w-6 h-6', ...config.textClass && {className: `w-6 h-6 ${config.textClass}`}})}
                <div>
                    <p className="text-sm text-gray-300">Current Status</p>
                    <p className={`font-bold text-lg ${config.textClass}`}>{village.status.charAt(0).toUpperCase() + village.status.slice(1)}</p>
                </div>
            </div>
                ...
                <div className="relative pl-5">
                    ...
                    {history.map((item, index) => (
                        <div key={index} className="relative pb-4 last:pb-0">
                            ...
                            <p className={`text-xs mt-1 ${statusConfig[item.status].textClass}`}>Status: {item.status}</p>
                        </div>
                    ))}
                </div>
            </div>
